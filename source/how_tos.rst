...
|                         |                                                                                                                                         |
+-------------------------+-----------------------------------------------------------------------------------------------------------------------------------------+
| job_xsp                 | Extra small high processing job size, ideal for jobs that heavily relies on find_outputs.                                               |
+-------------------------+-----------------------------------------------------------------------------------------------------------------------------------------+
...
+-------------------------+-----------------------------------------------------------------------------------------------------------------------------------------+
| job_xlp                 | Extra large high processing job size, ideal for jobs that heavily relies on find_outputs.                                               |
+-------------------------+-----------------------------------------------------------------------------------------------------------------------------------------+
|                         |                                                                                                                                         |
+-------------------------+-----------------------------------------------------------------------------------------------------------------------------------------+
...
|                         |                                                                                                                                         |
+-------------------------+-----------------------------------------------------------------------------------------------------------------------------------------+
| browser_fox_xs          | Extra small Firefox browser fetch size, ideal for Firefox browser fetch that uses heavy driver code.                                    |
+-------------------------+-----------------------------------------------------------------------------------------------------------------------------------------+
| browser_fox_s           | Small Firefox browser fetch size, ideal for Firefox browser fetch that uses heavier driver code.                                        |
+-------------------------+-----------------------------------------------------------------------------------------------------------------------------------------+
| browser_fox_m           | Medium Firefox browser fetch size, ideal for Firefox browser fetch that uses heavier driver code.                                       |
+-------------------------+-----------------------------------------------------------------------------------------------------------------------------------------+
| browser_fox_l           | Large Firefox browser fetch size, ideal for Firefox browser fetch that uses heavier driver code.                                        |
+-------------------------+-----------------------------------------------------------------------------------------------------------------------------------------+
| browser_fox_xl          | Extra large Firefox browser fetch size, ideal for Firefox browser fetch that uses heavier driver code.                                  |
+-------------------------+-----------------------------------------------------------------------------------------------------------------------------------------+
|                         |                                                                                                                                         |
+-------------------------+-----------------------------------------------------------------------------------------------------------------------------------------+
...
|                         |                                                                                                                                         |
+-------------------------+-----------------------------------------------------------------------------------------------------------------------------------------+
| fetcher_browserforge_xs | Extra small BrowserForge fetch size, ideal for BrowserForge fetch that uses heavy driver code.                                          |
+-------------------------+-----------------------------------------------------------------------------------------------------------------------------------------+
| fetcher_browserforge_s  | Small BrowserForge fetch size, ideal for BrowserForge fetch that uses heavier driver code.                                              |
+-------------------------+-----------------------------------------------------------------------------------------------------------------------------------------+
| fetcher_browserforge_m  | Medium BrowserForge fetch size, ideal for BrowserForge fetch that uses heavier driver code.                                             |
+-------------------------+-----------------------------------------------------------------------------------------------------------------------------------------+
| fetcher_browserforge_l  | Large BrowserForge fetch size, ideal for BrowserForge fetch that uses heavier driver code.                                              |
+-------------------------+-----------------------------------------------------------------------------------------------------------------------------------------+
| fetcher_browserforge_xl | Extra large BrowserForge fetch size, ideal for BrowserForge fetch that uses heavier driver code.                                        |
+-------------------------+-----------------------------------------------------------------------------------------------------------------------------------------+
| fetcher_browserforge_xxl| Extra extra large BrowserForge fetch size, ideal for BrowserForge fetch that uses heaviest driver code.                                 |
+-------------------------+-----------------------------------------------------------------------------------------------------------------------------------------+
|                         |                                                                                                                                         |
+-------------------------+-----------------------------------------------------------------------------------------------------------------------------------------+
...
Use BrowserForge headers
-------------------------------

**NOTE**: This feature needs that the scraper/job has BrowserForge enabled and a special profile configured (fetcher_browserforge_xs at least). 
The flag enables the BrowserForge python service internally and the profile provides the needed resources for this to work properly. Both are required for this feature to work properly.

To use BrowserForge headers, you can set the `use_browserforge` flag to `true` when enqueuing your page:
...
The default value for this flag is `false`. When you set it to true you can also can send the configuration by sending `browserforge_config`, the default values are the following if none is sent:

- **Supported mode**: `headers` (default value) and `fingerprint`
- **Supported Browsers**: `chrome`, `firefox`, `safari`, `edge` (default: if none sent one will be used at random)
- **Supported OS**: `windows`, `macos`, `linux`, `android`, `ios` (default: if none sent one will be used at random)
- **Supported Devices**: `desktop`, `mobile` (default: if none sent will be based on UA type sent)
   
For supported devices, the selection depends on the User-Agent type sent to the scraper:

- **desktop**: Sets device to "desktop"
- **mobile**: Sets device to "mobile"  
- **tablet**: Sets device to "mobile"
- **gb2**: Returns an error (gb2 is not a valid device type for BrowserForge)
- **default**: Sets device to "desktop" (for any other device type)

For other Options you can also send `locale`, `strict`, `request_dependent_headers`. You can read official docs to see what they do.

**NOTE**: if you set `strict` value as `true` and the configuration is not correct or header cannot be generated with the provided config will show an error.

We support a `cache_key` that will be use the same header information if the same key is used, preventing the tool to autogenerate it again and reuse if needed.

For any other options or configurations that we can support also check the BrowserForge documentation at https://pypi.org/project/browserforge/
and their source codes to check possible parameters that could also be sent here.

Take in consideration that some **fingerprint** options can be different from the **headers** options. 

Using mode **fingerprint** support these options: browser, os, device, locale, http_version, strict, user_agent, mock_webrtc, slim  and screen.

Using mode **headers** support these options:  browser, os, device, locale, http_version, strict, user_agent and request_dependent_headers.

Here is an example using **headers**:
...
.. code-block:: ruby
   ...
   pages << {
      ...
      browserforge_config: {
         mode: "headers",
         options: {
            # browser: "firefox", # single browser just sending a string value
            browser: '{"name": "chrome", "min_version": 110, "max_version": 120, "http_version": 2}', # more detailed browser configuration sending an object
            os: "macos",
            locale: "fr-FR",
            # strict: true, # remember that if this value is set to true will give an error if the tool cannot use the configuration for the generation
            request_dependent_headers: {
               Referer: "https://example.com"
            }
         },
         cache_key: "my-session-123"
      },
   }


Here is an example using **fingerprint**:
